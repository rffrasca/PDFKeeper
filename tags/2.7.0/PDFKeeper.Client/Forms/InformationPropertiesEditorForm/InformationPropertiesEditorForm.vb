	...
	Private Function ReadNextDocument() As Integer
		...
		Else
			...
			Dim oMessageDialog As New MessageDialog(String.Format( _
				...
				InformationPropertiesEditorForm_Strings.UnableRead, pdfFile))
	...
	Private Sub ButtonSaveClick(sender As Object, e As EventArgs)
		...
		If origin = "FORM" Then
		 	modPdfFile = pdfFile.Substring(0, (pdfFile.Length - 3)) & _
						 randomNum & ".pdf"
		ElseIf origin = "WATCHER" Then
			Dim oFileInfo As New FileInfo(pdfFile)
			savPdfFile = ArchiveDir & "\" & _
						 oFileInfo.Name.ToString.Substring(0, ( _
						 oFileInfo.Name.ToString.Length - 3)) & _
						 randomNum & ".A.pdf"
			modPdfFile = ArchiveDir & "\" & _
						 oFileInfo.Name.ToString.Substring(0, ( _
						 oFileInfo.Name.ToString.Length - 3)) & _
						 randomNum & ".B.pdf"
		End If
		Dim oPdfProperties As New PdfProperties(pdfFile, modPdfFile)
		...
		If oPdfProperties.Write(True) = 1 Then
			Me.Cursor = Cursors.Default
			Exit Sub
		Else
			...
			If checkBoxDeleteOrigAfterSave.Checked Then
				...
				Dim oMessageDialog2 As New MessageDialog(String.Format( _
					CultureInfo.CurrentCulture, _
					InformationPropertiesEditorForm_Strings.DeleteOriginal, _
					pdfFile))
				Dim oFileIO1 As New FileIO(pdfFile)
				Dim oFileIO2 As New FileIO(pdfFile, savPdfFile)
				If oMessageDialog2.DisplayQuestion = 6 Then	' "Yes" was selected.
					If oFileIO1.Delete = 1 Then
						If origin = "WATCHER" Then
							oFileIO2.Rename(True)
						End If
					End If
				Else
					oFileIO2.Rename(True)
			...
			If checkBoxAddUploadQueueAfterSave.Checked Then
				...
				If checkBoxDeleteSavedAfterUpload.Checked Then
					...
					Dim oMessageDialog1 As New MessageDialog(String.Format( _
						CultureInfo.CurrentCulture, _
						InformationPropertiesEditorForm_Strings.DeleteSaved, _
																modPdfFile))
					If oMessageDialog1.DisplayQuestion = 6 Then	' Yes
						MainForm.uploadDeleteSyncdQueue.Enqueue(modPdfFile)
					Else
						MainForm.uploadSyncdQueue.Enqueue(modPdfFile)
					End If
				Else
	...
	Private Sub ButtonCancelClick(sender As Object, e As EventArgs)
		...
		If QueueCount > 0 Then
			Dim oMessageDialog As New MessageDialog( _
				InformationPropertiesEditorForm_Strings.ContinueEditing)
