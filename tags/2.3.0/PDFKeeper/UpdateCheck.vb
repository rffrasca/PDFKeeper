...
Public Class UpdateCheck
	...
	Public Function IsUpdateAvailable() As Boolean
		...
		Using objWebClient As New WebClient
			Dim pageContents As String = objWebClient.DownloadString(updateURL)
			Dim matches As MatchCollection = Regex.Matches(pageContents, _
										"//\S+[^-,;:?]\.exe")
			For Each expMatch As Match In matches
				For Each objGroup As group In expMatch.Groups
					installerVersion = objGroup.Value.Substring( _
									   objGroup.Value.LastIndexOf("/", _
	...
	Public Sub OpenUpdateUrl
		...
		Else
			MessageDialog.Display("Error", "Unable to read update URL " & _
								  "from the registry.")
		End If
	End Sub
End Class
